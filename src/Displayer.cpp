...
void CDisplayer::gl_textureLoad(void)
{
	...
	for(winId=0; winId<m_renderCount; winId++)
	{
		...
		if(!((mask >> chId)&1))
		{
			...
			//stable		dism_img[chId].data
			if(gEnableStabFlag[chId])
			{
				cvGpuMat inFrame = cvGpuMat(dism_img[chId].rows, dism_img[chId].cols, dism_img[chId].type(), (void*)dism_img[chId].data, dism_img[chId].step.buf[0]);
				cvGpuMat outFrame = cvGpuMat(dism_img[chId].rows, dism_img[chId].cols, dism_img[chId].type(), (void*)dism_img[chId].data, dism_img[chId].step.buf[0]);
